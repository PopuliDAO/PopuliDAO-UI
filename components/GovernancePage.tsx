...
const GovernancePage = () => {
  ...
  const { writeContractAsync: writeYourContractAsync } = useWriteContract({
    address: '0xYourContractAddressHere', // Replace with your contract address
    abi: WorldverifyAbi/* ABI of your contract */, // Replace with your contract ABI
  ...
  const onSuccess = async (result: ISuccessResult) => {
    ...
    console.log('Function called');} catch (error) {
    ...
    <div className="flex flex-col items-center mt-4">
      <Card className="sm:col-span-2 p-4 min-w-7/12 w-9/12">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          >
            {({ open }) => (
              <Button onClick={open}>{"+ New Proposal"}</Button>
            )}
          </IDKitWidget>
        </CardHeader>
      </Card>
